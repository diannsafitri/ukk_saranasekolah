...
<head>
    ...
    <title>Login Admin - Sarana Sekolah</title>
    ...
    <div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-md w-full bg-white rounded-lg shadow-md p-8">
            ...
            <form method="POST" action="{{ route('admin.login') }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="username" name="username" value="{{ old('username') }}" required
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 @error('username') border-red-500 @enderror"
                        placeholder="admin">
                    @error('username')
            ...
            <p class="text-center text-gray-600 text-sm mt-6">
                Kembali ke <a href="{{ route('login') }}" class="text-blue-600 hover:text-blue-800 font-medium">login siswa</a>
            </p>
        </div>
    </div>
</body>
